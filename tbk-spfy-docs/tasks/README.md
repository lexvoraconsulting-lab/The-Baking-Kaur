...
- [ ] Implement the **Enterprise Collection Architecture** (2026-08-01, awaiting approval — see `docs/ENTERPRISE_COLLECTION_ARCHITECTURE.md`). Permanent target design: three-axis taxonomy (Occasion/Theme/Flavor), smart collections keyed on `product_type`+tags instead of today's fragile title-substring rules, navigation/breadcrumb/internal-linking blueprint, and a proposed `classify_product.py` automation script so future products self-classify. 6-phase roadmap; only the last phase (the duplicate-cluster resolution, shared with the item above) is business-gated — everything before it is additive/reversible.
