...
## 🔒 S1 — EDITORIAL HERO: **VERSION 1.2** — FROZEN (2026-07-16)
...
### v1.2 change — trust strip is now BLOCK-DRIVEN + structurally honest (2026-07-16)
Per client request, every trust fact is an independent **section block** — add / remove / reorder / edit in the theme editor, **no code change**. Four block types, two with structural honesty guards proven on preview:
| Block | Renders when | Guard |
|---|---|---|
| **Fact** | label present | plain verifiable fact (Eggless, Freshly Baked, Same-Day, Midnight) |
| **FSSAI licence** | always (shows number if given, else the label) | upgrades "FSSAI Licensed" → "FSSAI Lic. <number>" automatically when the number is entered |
| **Google rating** | **only if a Google Business Profile URL *and* a rating are both set** | 🔒 an unverified/unlinked rating is **impossible** — no URL → nothing renders. Shows as a link (`nofollow noopener`, new tab, aria-labelled). |
| **Customer count** | **only if 'Verified' is ticked *and* a count is set** | 🔒 an unverified count cannot display |

**Verified empirically (test blocks injected then reverted):** rating-without-URL hidden ✅ · rating-with-URL rendered as a link → source, `rel="nofollow noopener"`, aria-label ✅ · count-unverified hidden ✅ · count-verified rendered ✅.
**Consequence for the pending Google-rating restore:** it no longer needs a code change. When the client supplies the profile URL + rating + count, someone adds a **Google rating** block in the theme editor and it renders as a verified link automatically. A legacy `·`-separated `trust` string remains as a fallback (used only if zero blocks exist).
...
Re-frozen at v1.2 — no S1 changes without explicit unfreeze.
