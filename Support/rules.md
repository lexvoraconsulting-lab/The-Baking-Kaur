...
1. **Three-Tier Categorized Directory Hierarchy:** All repository automation tools reside under `tools-script/{win,python,mac}/` cleanly organized into functional domain subdirectories (`theme/`, `governance/`, `seo/`, `ai/`, `release/`, `tests/`, `cli/`). Flat script dumping in the repository root or unorganized script folders is strictly prohibited.
2. **Relative Root Traversal Depth:** Every script inside domain subdirectories must resolve repository root precisely three levels up (`%~dp0\..\..\..` for Windows batch, `$PSScriptRoot\..\..\..` for PowerShell, and `$SCRIPT_DIR/../../..` for bash).
3. **Resilient Theme Dev Server:** The theme dev server must resolve theme targets dynamically via Shopify CLI, auto-recover from stale theme IDs by falling back to active preview theme `#152070258857`, and auto-download assets if missing.
4. **Governing Records:** Traceable to Concept [`CON005`](concept-design/CON005_Unified_Developer_Toolchain_Master_Console_Concept.md), Research [`RSH_20260906_D`](research/RSH_20260906_D_developer_toolchain_centralized_runtime_and_console_ux.md), Architecture [`ARC_20260906_E`](architecture/ARC_20260906_E_developer_toolchain_and_operations_console.md), Decision [`DEC_20260906_B`](decisions/DEC_20260906_B_centralized_framework_toolchain_and_script_hierarchy.md), and Plan [`PLN005`](plans/02-operations/PLN005-developer-toolchain-and-master-operations-console-plan.md).
