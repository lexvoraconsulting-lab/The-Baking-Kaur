...
if(Shopify.designMode){
  ...
  */
  ...
  if (e.target && e.target.id === 'tbk-reference-images') {
...
document.addEventListener("change", function (e) {
  if (e.target.id === "tbk-reference-images") {
    ...
    const preview = document.getElementById("tbk-image-preview");
    if (!preview) return;
...
document.addEventListener("change", function (e) {
  if (e.target.id === "tbk-reference-images") {
    const files = e.target.files;
    const preview = document.getElementById("tbk-preview");
    preview.innerHTML = "";
